...
import pdfplumber
from graphs.algorithms.prim.prim import PrimAlgorithm
...
def prim_api(request):
    ...
    try:
        ...
        if not start_node or start_node not in nodes:
            return JsonResponse({"error": "Chọn đỉnh bắt đầu hợp lệ!"}, status=400)
        ...
        algo = PrimAlgorithm(G, start_node=start_node)
        result = algo.run()
...
def prim_page(request):
    return render(request, "graphs/algorithms_d3/prim/prim.html")

@csrf_exempt
def get_pdf_text_prim(request):
    pdf_path = os.path.join(settings.BASE_DIR, "graphs", "static", "graphs", "docs", "Prim_Intro.pdf")
