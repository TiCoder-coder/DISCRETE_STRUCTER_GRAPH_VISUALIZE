// Biến toàn cục kiểm tra hướng đồ thị
...
// ---------------------------------------------------------
// 1. CÁC HÀM RENDER BẢNG (An toàn - Kiểm tra dữ liệu từ Server)
// ---------------------------------------------------------
...
function renderAdjList(adjList) {
    ...
    `;
    // Đảm bảo duyệt qua các key theo thứ tự chữ cái (để hiển thị đẹp)
...
// ---------------------------------------------------------
// 2. HÀM CHÍNH: LẤY INPUT VÀ GỌI API (FETCH)
// ---------------------------------------------------------

/**
 * Hàm kiểm tra Đồ thị Hai phía (Bipartite). (Vẫn giữ coloring ở đây)
 */
function runBipartiteCheck() {
    ...
    .then(data => {
        ...
        // Chỉ hiển thị màu Hai phía khi gọi hàm này
...
function runBFS() {
    ...
    .then(data => {
        ...
        // 1. Vẽ đồ thị (reset màu về ban đầu - BỎ COLORING ĐỂ KHÔNG TÔ MÀU TÍM/XANH DA TRỜI TRƯỚC KHI DUYỆT)
        if (typeof drawGraph === "function") {
        ...
        if (result.steps && typeof animateBFS === "function") {
            // Chuẩn bị khung log trước khi gọi animateBFS
        ...
        // 3. Cập nhật bảng thông tin sau khi animation kết thúc
        const analysis = data.analysis || {};

        if (typeof updateInfo === "function") {
            // Dùng setTimeout để đảm bảo updateInfo chạy sau khi animateBFS hoàn tất
            setTimeout(() => {
                updateInfo({
                    ...analysis,
                    startNode: startNode,
...
/**
 * Vẽ đồ thị bằng D3.js. Cập nhật màu Hai phía và lưu màu gốc vào node data.
 */
function drawGraph(nodes, edges, coloring = {}) {
    ...
    if (window.isDirected) {
        ...
        defs.append("marker").attr("id", "arrow").attr("viewBox", "0 0 10 10")
            ...
            .attr("orient", "auto").append("path").attr("d", "M 0 0 L 10 5 L 0 10 z").attr("fill", "#00ff88");
    }
    ...
    // Tính toán originalFill và gắn vào dữ liệu nút
    const d3Nodes = nodes.map(n => {
        const node = (typeof n === 'object' ? n : { id: n });
        const colorVal = coloring[node.id];
        // Chỉ tô màu nếu coloring[node.id] là 0 hoặc 1, ngược lại dùng màu mặc định
        node.originalFill = "#1d2b57"; // Màu mặc định (Xanh đậm)
        if (colorVal === 0) node.originalFill = "#3498db"; // Màu Set 1 (Xanh da trời)
        if (colorVal === 1) node.originalFill = "#9b59b6"; // Màu Set 2 (Tím)
        return node;
    });
    ...
    if (window.isDirected) link.attr("marker-end", "url(#arrow)");

    // Sử dụng originalFill
    ...
    window.nodeElements = node; // Lưu trữ node elements

    // Thêm text riêng biệt và căn giữa
    ...
    function dragended(event, d) {
        ...
        d.fx = null; // Bỏ fx, fy để node trở lại simulation
        d.fy = null;
    }
    ...
    simulation.on("tick", () => {
        ...
        // Căn nhãn vào giữa đỉnh
        label.attr("x", d => d.x).attr("y", d => d.y + 6);
    });
}

/**
 * Chạy Animation cho BFS (chỉ dùng 2 màu Vàng/Xanh lam sáng)
 */
function animateBFS(result) {
    ...
    function stepLoop() {
        ...
        // SỬA LỖI LOG: Hiển thị Queue sau khi bước hiện tại (current_node) đã được xử lý
        // và đã thêm các đỉnh kề vào queue
        let logMessage = `<b>B${i + 1}:</b> Tại <b>${s.current_node}</b>`;

        if (s.action === 'DEQUEUE') {
             // Đỉnh được lấy ra, chỉ hiển thị đỉnh hiện tại và Queue sau khi lấy ra
        ...
        // Luôn hiển thị trạng thái Queue mới nhất
        ...
        // 1. Reset các nút đã thăm (visited_list) về màu XANH LAM SÁNG
        nodes.filter(d => s.visited_list.includes(d.id))
             .transition().duration(400).attr("fill", "#00e5ff").attr("r", 20);

        // 2. Highlight Node hiện tại (Vàng)
        nodes.filter(d => d.id === s.current_node)
             .transition().duration(400).attr("fill", "#f7d206").attr("r", 25);

        // 3. Highlight Edge đang xét (Đỏ)
        edges.attr("stroke", "#00ff88").attr("stroke-width", 3); // Reset các cạnh khác
        ...
        i++;
        setTimeout(stepLoop, 1000); // 1 giây cho mỗi bước
    }
...
/**
 * Cập nhật bảng thông tin sau khi animation BFS kết thúc hoặc khi kiểm tra Hai phía.
 */
function updateInfo(info) {
    ...
    const distance = info.distance || {}; // LẤY DỮ LIỆU BẬC TỪ SERVER

    // Xác định xem đây có phải là yêu cầu chỉ kiểm tra Hai phía không
    ...
    // --- LOGIC HIỂN THỊ BẬC (LEVELS) DƯỚI DẠNG BẢNG ---
    let levelsTableHtml = "";
    ...
    let stepsRows = "";
    (steps || []).forEach((s, i) => {
        // Queue hiển thị dạng: [A, B, C]
        const queueStr = `[${(s.queue || []).join(", ")}]`;
        const visitedStr = `[${(s.visited_list || []).join(", ")}]`; // Dùng visited_list từ API

        // Xác định đỉnh đang xét và tô màu
        ...
        if (s.action === 'DEQUEUE' || s.action === 'START') {
            ...
            currentNode = s.current_node; // Đang xét đỉnh kề, đỉnh hiện tại không cần highlight quá mức
        }
